<template>
  ...
  >
    <div v-if="buttonStyle" :style="inlineStyle ? 'display:inline-flex' : ''">
      <el-input
        v-if="!selectNid && options.length === 0"
        v-model="searchValue"
      ...
      <el-checkbox-button
        ...
        :key="list.id ? list.id : list[dictValue]"
        style="margin-right:2px"
        :label="list[dictValue]"
      >{{ list[dictName] }}</el-checkbox-button>
    </div>
    <div v-else>
      <el-checkbox
...
export default {
  ...
  methods: {
    init() {
      if (!this.selectNid && !this.dataUrl) {
        // 获取传递过来的数组
        this.getOptions();
      } else {
        this.loadDict();
